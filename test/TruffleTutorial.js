...
contract('TruffleTutorial', ([contractOwner, secondAddress, thirdAddress]) => {
  ...
  describe('message', () => {
    // check if owner can set a new message, check if setMessage works
  ...
  it('address that is not the owner fails to set a message', async() => {
    // `.should.be.rejected` tells Chai that the test should pass if the setMessage function fails
    await truffleTutorial.setMessage('Hey there!', { from: secondAddress }).should.be.rejected;

    await truffleTutorial.setMessage('Yolo!', { from: thirdAddress }).should.be.rejected;
  })
})